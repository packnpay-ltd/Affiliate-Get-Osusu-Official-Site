                ...
                <div class="container mx-auto px-4">
                    <div class="flex justify-between mb-6 items-center">
                        ...
                        <div class="flex gap-3">
                            ...
                            <a href="{{ route('products.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition-colors duration-200 flex items-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    @foreach ($products as $product)
                                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-600">{{ $product->brand->name }}</td>
                                            ...
                                            <td class="px-4 py-4 text-sm text-gray-600">₦{{ number_format($product->price, 2) }}</td>
                                            <td class="px-4 py-4">
                                                <img src="{{ asset('storage/' . $product->image) }}" alt="Product Image" class="w-12 h-12 object-cover rounded-lg">
                                            </td>
                                            <td class="px-4 py-4 text-sm">
                                                <div class="relative inline-block text-left">
                                                    ...
                                                    <div class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10 hidden" role="menu">
                                                        <div class="py-1">
                                                            <a href="{{ route('products.edit', $product->id) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                                ✏️ Edit
                                                            </a>
                                                            ...
                                                            <form action="{{ route('products.toggleFrontpage', $product->id) }}" method="POST" class="w-full">
                                                                @csrf
                                                                <button type="submit" class="w-full text-left px-4 py-2 text-sm {{ $product->frontpage ? 'text-red-600' : 'text-green-600' }} hover:bg-gray-100">
                                                                    {{ $product->frontpage ? '❌ Remove from Frontpage' : '✅ Add to Frontpage' }}
                                                                </button>
                                                            </form>
                                                            <form action="{{ route('products.togglePublished', $product->id) }}" method="POST">
                                                                @csrf
                                                                <button type="submit" class="block w-full text-left px-4 py-2 text-sm 
                                                                    {{ $product->is_published ? 'text-yellow-600' : 'text-green-600' }} hover:bg-gray-100">
                                                                    {{ $product->is_published ? '👁️ Unpublish' : '👁️ Publish' }}
                                                                </button>
                                                            </form>
                                                        </div>
        ...
        <div class="max-h-60 overflow-y-auto">
            @if($categoriesList && $categoriesList->count() > 0)
                @foreach($categoriesList as $category)
                    <div class="flex items-center justify-between py-2 border-b last:border-0">
                        <span class="text-gray-700">{{ $category->name ?? 'Unnamed Category' }}</span>
                        <div class="flex gap-2">
                ...
                @endforeach
            @else
                <p class="text-gray-500 text-center py-4">No categories found (Total: {{ $categories->count() ?? 0 }})</p>
            @endif
        </div>
    </div>
</div>
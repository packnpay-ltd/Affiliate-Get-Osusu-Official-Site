                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        ...
                        <p class="text-2xl font-bold">
                            {{ $referrals->count() > 0 ? number_format(($referrals->where('status', 'active')->count() / $referrals->count()) * 100, 2) : 0 }}%
                        </p>
                    </div>
                </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse($referrals as $referral)
                                                            <tr>
                                                                <td class="px-6 py-4 text-sm text-gray-500">{{ $referral->referredUser->name }}</td>
                                    ...
                                    ($referral->status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                                        'bg-red-100 text-red-800') }}">
                                                                ...
                                                                <td class="px-6 py-4 text-sm text-gray-900 text-right">
                                                                    ₦{{ number_format($referral->earnings->sum('amount'), 2) }}</td>
                                                            </tr>
                                @empty
                                    <tr>
                                        <td class="px-6 py-4 text-sm text-gray-500 text-center" colspan="4">No referrals yet
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
